...
export class StaticNotebook extends WindowedList {
     ...
     */
    onUpdateRequest(msg) {
        if (['defer', 'contentVisibility'].includes(this.notebookConfig.windowingMode)) {
    ...
    _onCellsChanged(sender, args) {
        ...
        switch (args.type) {
            ...
            case 'remove':
                ...
                this._updateDataWindowedListIndex(args.oldIndex, -1 * args.oldValues.length);
    ...
    _updateDataWindowedListIndex(start, delta) {
    ...
    async _runOnIdleTime(remainingTime = MAXIMUM_TIME_REMAINING) {
        ...
        while (Date.now() - startTime < remainingTime &&
            ...
            if (cell.isPlaceholder()) {
                if (['defer', 'full', 'contentVisibility'].includes(this.notebookConfig.windowingMode)) {
                    ...
                    else if (this.notebookConfig.windowingMode === 'contentVisibility') {
    ...
    _updateNotebookConfig() {
        ...
        const isContentVisibility = this._notebookConfig.windowingMode === 'contentVisibility';
        this.viewportNode.classList.toggle('jp-content-visibility-mode', isContentVisibility);
    ...
    _setupContentVisibilityObserver() {
        var _a;
        if (!this._contentVisibilityObserver) {
            // Create observer only once
            this._contentVisibilityObserver = new IntersectionObserver(entries => {
        ...
        this.cellsArray.forEach(cell => this._contentVisibilityObserver.observe(cell.node));
        // Watch for newly added cells and set intrinsic size for them too
        (_a = this.model) === null || _a === void 0 ? void 0 : _a.cells.changed.connect(() => {
            requestAnimationFrame(() => {
                this.cellsArray.forEach((cell, i) => {
                    const estHeight = this._viewModel.estimateWidgetSize(i);
                    cell.node.style.containIntrinsicSize = `auto ${estHeight}px`;
    ...
    async setFragment(fragment) {
        ...
        switch (parsedFragment.kind) {
            ...
            default:
                ...
                result = await this._findLegacy(parsedFragment.kind + '=' + parsedFragment.value);
                break;
        }
        if (result == null) {
            return;
    ...
    async _findHeading(queryId) {
        ...
        for (let cellIdx = 0; cellIdx < this.widgets.length; cellIdx++) {
            ...
            for (const heading of cell.headings) {
                ...
                if (id === queryId) {
                    const attribute = ((_a = this.rendermime.sanitizer.allowNamedProperties) !== null && _a !== void 0 ? _a : false)
                        ? 'id'
                        : 'data-jupyter-id';
                    const element = this.node.querySelector(`h${heading.level}[${attribute}="${CSS.escape(id)}"]`);
                    return {
                        cell,
                        element
                    };
    ...
    async _findLegacy(queryId) {
        ...
        for (let cellIdx = 0; cellIdx < this.widgets.length; cellIdx++) {
            ...
            const attribute = ((_a = this.rendermime.sanitizer.allowNamedProperties) !== null && _a !== void 0 ? _a : false)
                ? 'id'
                : 'data-jupyter-id';
            const element = cell.node.querySelector(`[${attribute}="${CSS.escape(queryId)}"]`);
            if (element !== null) {
                return {
                    cell,
                    element: element
